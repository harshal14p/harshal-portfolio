...
function makeGada(){
  const g=new THREE.Group();
  const metal=new THREE.MeshStandardMaterial({color:0x8bc8ff,metalness:.96,roughness:.18,transparent:true,opacity:.58,emissive:0x0b4b9c,emissiveIntensity:1.6});
  const glow=new THREE.MeshBasicMaterial({color:0x3b91ff,transparent:true,opacity:.13,blending:THREE.AdditiveBlending,depthWrite:false});
  const shaft=new THREE.Mesh(new THREE.CylinderGeometry(.075,.12,4.5,18),metal);shaft.position.y=-1.15;g.add(shaft);
  const grip=new THREE.Mesh(new THREE.CylinderGeometry(.14,.18,.58,18),metal);grip.position.y=-3.52;g.add(grip);
  const pommel=new THREE.Mesh(new THREE.SphereGeometry(.28,20,14),metal);pommel.scale.y=.55;pommel.position.y=-3.88;g.add(pommel);
  const collar=new THREE.Mesh(new THREE.TorusGeometry(.34,.065,10,28),metal);collar.rotation.x=Math.PI/2;collar.position.y=1.05;g.add(collar);
  const head=new THREE.Mesh(new THREE.SphereGeometry(.86,32,24),metal);head.scale.set(1,1.12,.96);head.position.y=1.75;g.add(head);
  const crown=new THREE.Mesh(new THREE.ConeGeometry(.6,.58,16),metal);crown.position.y=2.65;g.add(crown);
  const halo=new THREE.Mesh(new THREE.SphereGeometry(1.42,24,18),glow);halo.position.y=1.75;g.add(halo);
  const ring=new THREE.Mesh(new THREE.TorusGeometry(1.02,.035,8,40),glow);ring.rotation.x=Math.PI/2;ring.position.y=1.75;g.add(ring);
  return g;
}
function makeIcon(symbol){
  ...
  if(symbol==='movie'){
    ...
    const reel1=new THREE.Mesh(new THREE.TorusGeometry(.18,.045,8,20),line);reel1.position.set(-.3,.05,.13);g.add(reel1);const reel2=reel1.clone();reel2.position.x=.3;g.add(reel2);
  } else if(symbol==='tech'){const oct=new THREE.Mesh(new THREE.OctahedronGeometry(.62,1),mat);g.add(oct);const ring=new THREE.Mesh(new THREE.TorusGeometry(.88,.025,8,40),line);ring.rotation.x=.9;g.add(ring);
  } else if(symbol==='fashion'){const body=new THREE.Mesh(new THREE.CylinderGeometry(.18,.5,.95,12),mat);body.position.y=-.05;g.add(body);const collar=new THREE.Mesh(new THREE.TorusGeometry(.3,.045,8,20),line);collar.rotation.x=Math.PI/2;collar.position.y=.43;g.add(collar);
  } else if(symbol==='music'){const stem=new THREE.Mesh(new THREE.BoxGeometry(.07,1.05,.07),mat);stem.position.set(.22,.15,0);g.add(stem);const note=new THREE.Mesh(new THREE.TorusGeometry(.23,.055,8,20),line);note.rotation.x=Math.PI/2;note.position.set(-.02,-.35,0);g.add(note);
  } else {const outer=new THREE.Mesh(new THREE.BoxGeometry(1.1,.7,.22),mat);g.add(outer);const d1=new THREE.Mesh(new THREE.TorusGeometry(.23,.045,8,20),line);d1.rotation.y=Math.PI/2;d1.position.set(-.28,0,.16);g.add(d1);const d2=d1.clone();d2.position.x=.28;g.add(d2)}
  return g;
}
...
function enhanceUI(){const contact=document.getElementById('contact');if(contact&&!contact.querySelector('.final-convergence')){const wrap=document.createElement('div');wrap.className='final-convergence';wrap.innerHTML='<span class="final-kicker">THE WORLD ENDS HERE.</span><h2>HARSHAL<br>CHOUHAN</h2><p>LET’S BUILD SOMETHING WORTH REMEMBERING.</p><div class="final-line"></div>';contact.prepend(wrap);contact.classList.add('cinematic-ending')}}
...
export function initScene(){
  ...
  gada=makeGada();gada.position.set(-4.7,2.2,-3.5);gada.rotation.set(.08,-.32,.12);gada.scale.setScalar(.88);scene.add(gada);
  ...
  const key=new THREE.PointLight(0x3c94ff,12,25);key.position.set(4,4,8);scene.add(key);const fill=new THREE.PointLight(0x6cb7ff,5,20);fill.position.set(-7,-2,5);scene.add(fill);scene.add(new THREE.AmbientLight(0x183050,.65));
  ...
  const clock=new THREE.Clock();const tick=()=>{const t=clock.getElapsedTime();pointer.lerp(target,.035);scrollCurrent+=(scrollTarget-scrollCurrent)*.025;
    ...
    gada.rotation.y=-.32+pointer.x*.28+Math.sin(t*.2)*.035;gada.rotation.z=.12+pointer.y*.12;gada.rotation.x=.08+scrollCurrent*.72;gada.position.y=2.2-scrollCurrent*5.8;gada.position.x=-4.7+pointer.x*.65;
    ...
    camera.position.x+=(pointer.x*1.2-camera.position.x)*.025;camera.position.y+=(-pointer.y*.65-camera.position.y)*.025;camera.position.z+=(14+scrollCurrent*1.5-camera.position.z)*.02;camera.lookAt(0,0,0);
