...
const clean=document.createElement('link');clean.rel='stylesheet';clean.href='css/clean-ui-overrides.css?v=1';document.head.appendChild(clean);
...
/* Decorative HUD/section-code overlays intentionally disabled. */
...
function enhanceSkills(){const grid=document.querySelector('.skills-grid');if(!grid||grid.dataset.cxReady)return;grid.dataset.cxReady='1';grid.classList.add('cx-skill-system');grid.querySelectorAll('.skill-group').forEach(el=>el.classList.add('cx-skill-orbit'))}
