...
const link=document.createElement('link');link.rel='stylesheet';link.href='css/cinematic-enhancement.css?v=4';document.head.appendChild(link);
const clean=document.createElement('link');clean.rel='stylesheet';clean.href='css/clean-ui-overrides.css?v=2';document.head.appendChild(clean);
...
function enhanceWorkflow(){document.querySelectorAll('.stage').forEach((el,i)=>el.style.setProperty('--stage-delay',`${i*90}ms`))}
function enhanceExperience(){document.querySelectorAll('.timeline-item').forEach(el=>el.classList.add('cx-career-node'))}
function enhanceSkills(){const grid=document.querySelector('.skills-grid');if(!grid||grid.dataset.cxReady)return;grid.dataset.cxReady='1';grid.querySelectorAll('.skill-group').forEach(el=>el.classList.add('cx-skill-orbit'))}
...
enhanceWorkflow();enhanceExperience();enhanceSkills();enhanceProjects();enhanceContact();
...
function frame(){rx+=(tx-rx)*.035;ry+=(ty-ry)*.035;world.style.transform=`translate3d(${rx*-8}px,${ry*-5}px,0)`;document.documentElement.style.setProperty('--cx-mx',`${rx*18}px`);document.documentElement.style.setProperty('--cx-my',`${ry*12}px`);requestAnimationFrame(frame)}requestAnimationFrame(frame);
