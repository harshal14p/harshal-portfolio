...
load("./galaxy.js?v=11","initGalaxy",true);
...
async function loadSelf(path){try{await import(path);}catch(error){console.warn(`[Portfolio] optional feature skipped: ${path}`,error);showContent();}}
...
function initContactActions(){
  ...
  actions.querySelectorAll('a').forEach(el=>{const href=(el.getAttribute("href")||"").toLowerCase();const text=(el.textContent||"").trim().toLowerCase();if(text.includes("download resume")&&!el.matches('[data-bind="resume-link"]'))el.setAttribute("data-bind","resume-link");if(href.includes("wa.me"))el.remove()});
  ...
  let wa=actions.querySelector('.contact-whatsapp');if(!wa){wa=document.createElement("a");actions.appendChild(wa)}
