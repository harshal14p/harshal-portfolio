...
function addHanumanReference(){if(document.querySelector('.hanuman-reference'))return;const host=document.querySelector('.hero')||document.body;const ref=document.createElement('div');ref.className='hanuman-reference';ref.setAttribute('aria-hidden','true');ref.innerHTML='<span class="gada-base"></span><span class="hanuman-reference-label">SYMBOL / STRENGTH</span>';host.appendChild(ref);}
function init(){applyConfig();applySEO();loadStyle('css/regenerated-ui.css');loadStyle('css/premium-overhaul.css');loadStyle('css/final-art-direction.css');loadStyle('css/final-polish.css');initNav();initTimeline();initContact();initProjects();initVideoReview();initPremium();init3D();addHanumanReference();document.querySelectorAll('.reveal').forEach(el=>el.classList.add('is-visible'));}
