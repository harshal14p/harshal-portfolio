...
function initPhotoMotion(){const photo=document.querySelector('.hero-photo');if(!photo)return;const inner=photo.querySelector('.hero-photo-inner');if(!inner)return;const reset=()=>{inner.style.setProperty('--photo-rx','0deg');inner.style.setProperty('--photo-ry','0deg');inner.style.setProperty('--shine-x','-120%')};photo.addEventListener('pointermove',e=>{const r=photo.getBoundingClientRect();const x=(e.clientX-r.left)/r.width-.5;const y=(e.clientY-r.top)/r.height-.5;inner.style.setProperty('--shine-x',`${50+x*95}%`);inner.style.setProperty('--photo-rx',`${-y*1.15}deg`);inner.style.setProperty('--photo-ry',`${x*1.55}deg`);});photo.addEventListener('pointerleave',reset);reset();}
