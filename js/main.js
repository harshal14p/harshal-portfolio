...
async function load(path,fn){try{const mod=await import(path);if(typeof mod[fn]==="function")mod[fn]();}catch(error){console.warn(`[Portfolio] optional module skipped: ${path}`,error);showContent();}}
...
function applySEO(){
  ...
  setMeta("og:title",title,true);setMeta("og:description",description,true);setMeta("og:url",base,true);setMeta("og:image",new URL(SITE.profileImagePath,base).href,true);
  setMeta("twitter:title",title);setMeta("twitter:description",description);setMeta("twitter:image",new URL(SITE.profileImagePath,base).href);
  const canonical=document.querySelector('link[rel="canonical"]');if(canonical)canonical.href=base;
}
...
function initContactActions(){document.querySelectorAll("[data-action=call]").forEach(el=>el.addEventListener("click",()=>{const n=String(el.dataset.phone||"").replace(/\D/g,"");if(n)window.location.href=`tel:${n}`}));document.querySelectorAll("[data-action=email]").forEach(el=>el.addEventListener("click",()=>{const e=el.dataset.email||SITE.email;if(e)window.location.href=`mailto:${e}`}))}
